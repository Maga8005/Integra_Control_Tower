...
export default function FKCSVDataViewer({ className }: FKCSVDataViewerProps) {
  ...
  const toggleRowExpansion = (operacionId: string) => {
    const newExpanded = new Set(expandedRows);
    if (newExpanded.has(operacionId)) {
      newExpanded.delete(operacionId);
    } else {
      newExpanded.add(operacionId);
    }
...
interface CSVDataTabProps {
  ...
  onToggleRowExpansion: (operacionId: string) => void;
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {csvData.slice(0, 20).map((row, index) => {
              ...
              // Extraer ID único de operación de la columna "Nombre"
              const operacionId = row['Nombre'] || `row-${index}`;
              
              return (
                <tr key={operacionId} className="hover:bg-gray-50">
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                    <button
                      onClick={() => onToggleRowExpansion(operacionId)}
                      className="flex items-center gap-1 hover:text-primary-600"
                    >
                      {expandedRows.has(operacionId) ? (
