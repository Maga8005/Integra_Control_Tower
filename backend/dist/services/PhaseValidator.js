...
exports.validateCompleteTimeline = validateCompleteTimeline;
 ...
 */
function validateCompleteTimeline(context) {
 ...
 */
function validateIndividualPhase(phaseNumber, context) {
    const warnings = [];
    const errors = [];
    const suggestions = [];
 ...
 */
function validatePhase1Details(phase, context) {
    const warnings = [];
    const errors = [];
    const suggestions = [];
    ...
    if (phase.progress === 100 && phase.estado !== Operation_1.EstadoProceso.COMPLETADO) {
        warnings.push({
            phase: 1,
            type: 'inconsistency',
 ...
 */
function validatePhase2Details(phase, context) {
    const warnings = [];
    const errors = [];
    const suggestions = [];
 ...
 */
function validatePhase3Details(phase, context) {
    const warnings = [];
    const errors = [];
    const suggestions = [];
    ...
    if (giros.length === 0 && phase.estado !== Operation_1.EstadoProceso.PENDIENTE) {
        warnings.push({
            phase: 3,
            type: 'data_quality',
 ...
 */
function validatePhase4Details(phase, context) {
    const warnings = [];
    const errors = [];
    const suggestions = [];
 ...
 */
function validatePhase5Details(phase, context) {
    const warnings = [];
    const errors = [];
    const suggestions = [];
    const liberaciones = context.parsedInfo.liberaciones || [];
 ...
 */
function validatePhaseDependencies(context) {
    const warnings = [];
    const errors = [];
    const suggestions = [];
    ...
    for (let i = 1; i < phases.length; i++) {
        ...
        if (!currentPhase.dependencies && currentPhase.estado !== Operation_1.EstadoProceso.PENDIENTE) {
 ...
 */
function validateLogicalCoherence(context) {
    const warnings = [];
    const errors = [];
    const suggestions = [];
    const phases = context.phaseDetails;
    ...
    const completedPhases = phases.filter(p => p.estado === Operation_1.EstadoProceso.COMPLETADO).length;
    ...
    const hasCompletedAfterPending = phases.some((phase, index) => {
        if (index === 0)
            return false;
        return phase.estado === Operation_1.EstadoProceso.COMPLETADO &&
            phases.slice(0, index).some(p => p.estado === Operation_1.EstadoProceso.PENDIENTE);
    });
 ...
 */
function validateDataQuality(context) {
    const warnings = [];
    const errors = [];
    const suggestions = [];
