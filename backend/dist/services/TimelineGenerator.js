 ...
 */
function generateTimelineEvent(phase, index, csvRow, parsedInfo, estados, paymentProgress, liberationValidation) {
 ...
 */
function generatePhase1Logic(csvRow, estados) {
 ...
 */
function generatePhase2Logic(csvRow, estados) {
    ...
    if (estados.cotizacion === Operation_1.EstadoProceso.COMPLETADO) {
        return {
            estado: Operation_1.EstadoProceso.EN_PROCESO,
            progreso: 30,
 ...
 */
function generatePhase3Logic(csvRow, estados, paymentProgress) {
 ...
 */
function generatePhase4Logic(csvRow, estados, parsedInfo) {
 ...
 */
function generatePhase5Logic(csvRow, estados, liberationValidation, parsedInfo) {
    ...
    const fase3 = generatePhase3Logic(csvRow, estados, (0, StateMapper_1.calculatePaymentProgress)(parsedInfo));
    const fase4 = generatePhase4Logic(csvRow, estados, parsedInfo);
    const fasesAnteriores = [fase1, fase2, fase3, fase4];
    const fasesCompletadas = fasesAnteriores.filter(fase => fase.estado === Operation_1.EstadoProceso.COMPLETADO);
    const fasesEnProceso = fasesAnteriores.filter(fase => fase.estado === Operation_1.EstadoProceso.EN_PROCESO);
 ...
 */
function calculateOverallProgress(timeline) {
...
function getTimelineSummary(timeline) {
    ...
    timeline.forEach((event, index) => {
        switch (event.estado) {
            case Operation_1.EstadoProceso.COMPLETADO:
                completedPhases++;
                break;
            case Operation_1.EstadoProceso.EN_PROCESO:
                inProgressPhases++;
                if (!currentPhase)
                    currentPhase = event.fase;
                break;
            case Operation_1.EstadoProceso.PENDIENTE:
                pendingPhases++;
                if (!nextPhase && !currentPhase)
                    nextPhase = event.fase;
                break;
        }
    });
    return {
 ...
 */
function validateTimelineCoherence(timeline) {
    const issues = [];
    for (let i = 1; i < timeline.length; i++) {
        ...
        if (currentEvent.estado === Operation_1.EstadoProceso.COMPLETADO &&
            previousEvent.estado === Operation_1.EstadoProceso.PENDIENTE) {
        ...
        if (previousEvent.estado !== Operation_1.EstadoProceso.COMPLETADO &&
