 ...
 */
export function calculatePreciseProgress(csvRow: CSVRow, parsedInfo: ParsedOperationInfo): OverallProgress {
  console.log('🧮 Calculando progreso preciso de 5 fases...');

  const phaseDetails: PhaseProgress[] = [
  ...
  const completedPhases = phaseDetails.filter(p => p.estado === EstadoProceso.COMPLETADO).length;
 ...
 */
function calculatePhase1Progress(csvRow: CSVRow): PhaseProgress {
 ...
 */
function calculatePhase2Progress(csvRow: CSVRow): PhaseProgress {
 ...
 */
function calculatePhase3Progress(csvRow: CSVRow, parsedInfo: ParsedOperationInfo): PhaseProgress {
 ...
 */
function calculatePhase4Progress(csvRow: CSVRow, parsedInfo: ParsedOperationInfo): PhaseProgress {
 ...
 */
function calculatePhase5Progress(csvRow: CSVRow, parsedInfo: ParsedOperationInfo): PhaseProgress {
 ...
 */
function calculatePaymentProgressDetailed(parsedInfo: ParsedOperationInfo): {
  totalAmount: number;
  paidAmount: number;
 ...
 */
function validateLiberationCompletion(parsedInfo: ParsedOperationInfo, tolerance: number = 1000): {
  isComplete: boolean;
  totalLiberated: number;
 ...
 */
function calculateWeightedProgress(phases: PhaseProgress[]): number {
 ...
 */
function determineCurrentPhase(phases: PhaseProgress[]): {
  currentPhase: number | null;
  nextPhase: number | null;
} {
  ...
  for (let i = 0; i < phases.length; i++) {
    ...
    if (phase.estado === EstadoProceso.EN_PROCESO && currentPhase === null) {
      currentPhase = i + 1;
    }
    
    if (phase.estado === EstadoProceso.PENDIENTE && nextPhase === null) {
      nextPhase = i + 1;
      break;
    }
  }
 ...
 */
function validatePhaseDependencies(phases: PhaseProgress[]): void {
